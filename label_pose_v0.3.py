...
image_path = r"C:\Users\李子贺\Pictures\4.9 棒球"

label_path = r"C:\Users\李子贺\Pictures\4.9 棒球\新建文件夹"
dirs = os.listdir(image_path)

def draw_point(img, point, flag, number = 0):
    size = max(img.shape) // 400
    if flag == 2:
        if number > 0:
            cv2.putText(image, str(number), point, cv2.FONT_HERSHEY_SIMPLEX, 0.75, (255, 0, 0), size)
        cv2.circle(img, point, 2, (255, 0, 0), size)
    elif flag == 1:
        if number > 0:
            cv2.putText(image, str(number), point, cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 255, 0), size)
        cv2.circle(image, point, 2, (0, 255, 0), size)
    elif flag == 0:
        if number > 0:
            cv2.putText(image, str(number), point, cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 255), size)
        cv2.circle(image, point, 2, (0, 0, 255), size)
...
def draw_box(img, p1, p2, label):
    size = max(img.shape) // 400
    if label == 'l':
        cv2.rectangle(image, p1, p2, (255, 0, 0), thickness=size)
    elif label == 'r':
        cv2.rectangle(image, p1, p2, (0, 255, 0), thickness=size)
    return img
...
def on_mouse(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_MBUTTONDOWN:  # 中键点击
        ...
        if point2 == point1 or np.abs(point2[0] - point1[0]) < 50 or np.abs(point2[1] - point1[1]) < 50:            # 点
            ...
            #draw_point(image, point, 0, points.__len__())
            cv2.imshow(window_name, image)
